          ...
          <CommandList>
            ...
            <CommandGroup>
              <CommandItem
                ...
                onSelect={() => {
                  if (!isNameUsed) {
                    // If matching template found, use it, else generic create
                    // Actually, if user types something, we should probably allow them to use that custom name
                    // But TierNameSelector typically selects from templates OR allows creating new.
                    // For now, let's assume if input has value, we treat it as selecting a custom name if "create" clicked.
                    if (inputValue) {
                        onChange(inputValue);
                    }
                    setOpen(false);
